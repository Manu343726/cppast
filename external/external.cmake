...
function(_cppast_find_libclang config_tool min_version force)
    ...
    if(NOT LIBCLANG_INCLUDE_DIR)
        if(CPPAST_USING_CONAN AND CONAN_INCLUDE_DIRS_CLANG)
            set(include_dir "${CONAN_INCLUDE_DIRS_CLANG}")
        else()
            execute_process(COMMAND ${config_tool} --includedir
                            OUTPUT_VARIABLE include_dir OUTPUT_STRIP_TRAILING_WHITESPACE)
        endif()
        find_path(LIBCLANG_INCLUDE_DIR "clang-c/Index.h" "${include_dir}" NO_DEFAULT_PATH)
    ...
    if(NOT LIBCLANG_LIBRARY)
        if(CPPAST_USING_CONAN AND CONAN_LIB_DIRS_CLANG)
            set(library_dir "${CONAN_LIB_DIRS_CLANG}")
        else()
            execute_process(COMMAND ${config_tool} --libdir
                            OUTPUT_VARIABLE library_dir OUTPUT_STRIP_TRAILING_WHITESPACE)
        endif()
        find_library(LIBCLANG_LIBRARY "clang" "${library_dir}" NO_DEFAULT_PATH)
    ...
    if(NOT LIBCLANG_SYSTEM_INCLUDE_DIR)
        find_path(LIBCLANG_SYSTEM_INCLUDE_DIR "stddef.h" "${library_dir}/clang/${llvm_version}/include" NO_DEFAULT_PATH)
    ...
    if(NOT CLANG_BINARY)
        if(CPPAST_USING_CONAN AND CONAN_BIN_DIRS_CLANG)
            set(binary_dir "${CONAN_BIN_DIRS_CLANG}")
        else()
            execute_process(COMMAND ${config_tool} --bindir
                            OUTPUT_VARIABLE binary_dir OUTPUT_STRIP_TRAILING_WHITESPACE)
        endif()
        find_program(CLANG_BINARY "clang" "${binary_dir}" NO_DEFAULT_PATH)
